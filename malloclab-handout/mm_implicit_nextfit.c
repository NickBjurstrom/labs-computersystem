...
static char *mm_heap;            /*a pointer to the first block */
static char *next_fit;           /* Next fit*/
 ...
 */
 static void *find_firstfit(size_t size)
 {
    ...
    void *bp;
    char *oldnext_fit = next_fit;

    /*search from next_fit*/
    for ( ; GET_SIZE(HEADER(next_fit)) > 0; next_fit = NEXT_BP(next_fit))
        if (!GET_ALLOC(HEADER(next_fit)) && (size <= GET_SIZE(HEADER(next_fit))))
            return next_fit;


    /* search from start to old next fit*/
        for (next_fit = mm_heap; next_fit < oldnext_fit; next_fit = NEXT_BP(next_fit)){
            if (!GET_ALLOC(HEADER(next_fit)) && (size <= GET_SIZE(HEADER(next_fit)))) {
                return next_fit;
            }
        }
 ...
 */
 static void *coalesce(void *bp)
 {
    ...
    /* modify next fit, let it point to a used block*/
    if((next_fit > (char *) bp) && (next_fit < NEXT_BP(bp)))
        next_fit = bp;
    }
